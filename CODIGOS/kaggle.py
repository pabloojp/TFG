...
# Realizamos una normalización de escala de grises para reducir el efecto de las diferencias de iluminación. Además, las CNN convergen más rápidamente con datos en el rango [0..1] que en el rango [0..255].
X_train = X_train / 255.0
test = test / 255.0


# Esta parte del código se encarga de remodelar las imágenes a 3 dimensiones y un solo canal (escala de grises). Cada imagen se representa como una matriz de píxeles con dimensiones 28x28 píxeles. Se preparan los datos para procesarlos en una (CNN), que espera imágenes con estas dimensiones.
X_train = X_train.values.reshape(-1,28,28,1)
test = test.values.reshape(-1,28,28,1)


# Se codifican las etiquetas en vectores "one-hot". Esto es necesario cuando se trabaja con problemas de clasificación multiclase, como el nuestro. La idea es representar cada etiqueta como un vector binario en el que un solo elemento tiene un valor de 1 y los demás son 0.
Y_train = to_categorical(Y_train, num_classes = 10)


# Establecemos la semilla aleatoria en 2
random_seed = 2


# Dividimos el conjunto de entrenamiento en dos partes: la validación (10%) y el entreno (90%), para asegurar que el modelo pueda ser evaluado en datos que no ha visto durante el entrenamiento. La división aleatoria garantiza que no haya un desequilibrio en las etiquetas entre el conjunto de entrenamiento y el conjunto de validación. 
